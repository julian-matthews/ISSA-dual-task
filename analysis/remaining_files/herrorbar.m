function hh = herrorbar(x, y, l, u,symbol,no_tee)
...
if nargin > 2
    if isstr(l)
        no_tee = u; % Account for no_tee choice if no u/l input
    elseif isstr(u)
        no_tee = symbol; % Account for no_tee choice if no u/l input
    end
    if nargin == 5
        if isstr(symbol)
            no_tee = 0; % Default 'tee' is on (added by Julian 2016-09-12)
        end
    end
end
...
tee = mean([u; l])/4; % Added by Julian (08/01/16) - standard 'tee' size
% changed from errorbar.m

% Added by Julian (13/06/16) - remove 'tee' 
if no_tee
    tee = 0;
end
