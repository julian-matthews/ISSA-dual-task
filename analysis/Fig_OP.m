%% FIG_OP: Creates Type-I AUC plots
%
...
function Fig_OP(Data,save_flag,PDT_flag,path_flag)
%% GROUP OBJECTIVE PERFORMANCE PLOT
...
num_runs = size(Data.ST_Central.Objective_Performance_SubByRun,2);
...
for sub = 1:Data.Subjects
    ...
    max_vector = [Data.ST_Peripheral.Objective_Performance_SubByRun(sub,:),...
        Data.ST_Central.Objective_Performance_SubByRun(sub,:),...
        Data.DT_Peripheral.Objective_Performance_SubByRun(sub,:),...
        Data.DT_Central.Objective_Performance_SubByRun(sub,:)];
    ...
    SINGLE_PERIPHERAL = Data.ST_Peripheral.Objective_Performance_MEAN(sub);
    SINGLE_CENTRAL = Data.ST_Central.Objective_Performance_MEAN(sub);
    DUAL_PERIPHERAL = Data.DT_Peripheral.Objective_Performance_MEAN(sub);
    DUAL_CENTRAL = Data.DT_Central.Objective_Performance_MEAN(sub);
    ...
    sh = scatter(line_fill,Data.ST_Peripheral.Objective_Performance_SubByRun(sub,:),...
        mark*8.5,gradData,'filled');
    set(sh,'MarkerEdgeColor','k');
    
    h = errorbar(mean_coord, Data.ST_Peripheral.Objective_Performance_MEAN(sub),...
        Data.ST_Peripheral.Objective_Performance_SEM(sub),'ko');
    ...
    sh = scatter(Data.ST_Central.Objective_Performance_SubByRun(sub,:),line_fill,...
        mark*8.5,gradData,'filled');
    set(sh,'MarkerEdgeColor','k');
    
    h = herrorbar(Data.ST_Central.Objective_Performance_MEAN(sub), mean_coord,...
        Data.ST_Central.Objective_Performance_SEM(sub),'ko',clear_tees);
    set(h,'MarkerSize',mark+2,'LineWidth',2)
    set(h,'MarkerFaceColor','w');
    
    if sub == 7
        % set(gca, 'FontSize', 16);
        xlabel('Central Type-I AUC');
    end
    
    % Dual Data
    
    % Scatter of participant's dual-task data
    sh(1) = scatter(Data.DT_Central.Objective_Performance_SubByRun(sub,:),...
        Data.DT_Peripheral.Objective_Performance_SubByRun(sub,:),...
        mark*8.5,gradData,'filled');
    set(sh,'MarkerEdgeColor','k');
    
    % 'Target' point representing dual-task means along with SEMs
    h = errorbar(Data.DT_Central.Objective_Performance_MEAN(sub), Data.DT_Peripheral.Objective_Performance_MEAN(sub),...
        Data.DT_Peripheral.Objective_Performance_SEM(sub),'ko');
    set(h,'MarkerSize',mark+2,'LineWidth',2);
    set(h,'MarkerFaceColor','w');
    ...
    h = herrorbar(Data.DT_Central.Objective_Performance_MEAN(sub), Data.DT_Peripheral.Objective_Performance_MEAN(sub),...
        Data.DT_Central.Objective_Performance_SEM(sub),'ko',clear_tees);
    set(h,'MarkerSize',mark+2,'LineWidth',2)
    set(h,'MarkerFaceColor','w');
    ...
    if PDT
        ...
        sh(1) = scatter(Data.PDT_Central.Objective_Performance_SubByRun(sub,:),...
            Data.PDT_Peripheral.Objective_Performance_SubByRun(sub,:),...
            mark*8.5,gradData,'filled','s');
        set(sh,'MarkerEdgeColor','k');
        
        % 'Target' point representing partial dual-task means along with SEMs
        h = errorbar(Data.PDT_Central.Objective_Performance_MEAN(sub), Data.PDT_Peripheral.Objective_Performance_MEAN(sub),...
            Data.PDT_Peripheral.Objective_Performance_SEM(sub),'ks');
        set(h,'MarkerSize',mark+2,'LineWidth',2);
        set(h,'MarkerFaceColor','w');
        ...
        h = herrorbar(Data.PDT_Central.Objective_Performance_MEAN(sub), Data.PDT_Peripheral.Objective_Performance_MEAN(sub),...
            Data.PDT_Central.Objective_Performance_SEM(sub),'ks',clear_tees);
        set(h,'MarkerSize',mark+2,'LineWidth',2)
        set(h,'MarkerFaceColor','w');
...
%% PLOT GROUP DATA (1,1) - OBJECTIVE PERFORMANCE
% Calculated separately for each block - points gradient shifted over time
% Single-task results presented along (x,y) axes, dual-task within centre
...
% Find edges of axis, bit greater than ST or DT overall maximum
max_vector = [mean(Data.ST_Peripheral.Objective_Performance_SubByRun,1),...
    mean(Data.ST_Central.Objective_Performance_SubByRun,1),...
    mean(Data.DT_Peripheral.Objective_Performance_SubByRun,1),...
    mean(Data.DT_Central.Objective_Performance_SubByRun,1)];
...
% Comparison 'triangle' ranging from single-task means
% Final 'line' connecting the triangle defined by limits
SINGLE_PERIPHERAL = mean(Data.ST_Peripheral.Objective_Performance_MEAN);
SINGLE_CENTRAL = mean(Data.ST_Central.Objective_Performance_MEAN);
DUAL_PERIPHERAL = mean(Data.DT_Peripheral.Objective_Performance_MEAN);
DUAL_CENTRAL = mean(Data.DT_Central.Objective_Performance_MEAN);
...
line([chance_value SINGLE_CENTRAL],[SINGLE_PERIPHERAL chance_value],'Color',coord_col,'LineWidth',axis_width);
...
sh = scatter(line_fill,(mean(Data.ST_Peripheral.Objective_Performance_SubByRun,1)),...
    mark*8.5,gradData,'filled');
set(sh,'MarkerEdgeColor','k');

h = errorbar(mean_coord, mean(Data.ST_Peripheral.Objective_Performance_MEAN),...
    mean(Data.ST_Peripheral.Objective_Performance_SEM),'ko');
set(h,'MarkerSize',mark+2,'LineWidth',2);
set(h,'MarkerFaceColor','w');
...
sh = scatter(mean(Data.ST_Central.Objective_Performance_SubByRun,1),line_fill,...
    mark*8.5,gradData,'filled');
set(sh,'MarkerEdgeColor','k');

h = herrorbar(mean(Data.ST_Central.Objective_Performance_MEAN), mean_coord,...
    Data.ST_Central.Objective_Performance_SEM(sub),'ko',clear_tees);
set(h,'MarkerSize',mark+2,'LineWidth',2)
set(h,'MarkerFaceColor','w');
...
% Scatter of participant's dual-task data
dualdouble(1) = scatter(mean(Data.DT_Central.Objective_Performance_SubByRun,1),...
    mean(Data.DT_Peripheral.Objective_Performance_SubByRun,1),...
    mark*8.5,gradData,'filled');
set(dualdouble,'MarkerEdgeColor','k');

% 'Target' point representing dual-task means along with SEMs
h = errorbar(mean(Data.DT_Central.Objective_Performance_MEAN), mean(Data.DT_Peripheral.Objective_Performance_MEAN),...
    mean(Data.DT_Peripheral.Objective_Performance_SEM),'ko');
set(h,'MarkerSize',mark+2,'LineWidth',2);
set(h,'MarkerFaceColor','w');
...
h = herrorbar(mean(Data.DT_Central.Objective_Performance_MEAN), mean(Data.DT_Peripheral.Objective_Performance_MEAN),...
    mean(Data.DT_Central.Objective_Performance_SEM),'ko',clear_tees);
set(h,'MarkerSize',mark+2,'LineWidth',2)
set(h,'MarkerFaceColor','w');
...
if PDT
    ...
    % Scatter of participant's partial dual-task data
    dualsolo(1) = scatter(mean(Data.PDT_Central.Objective_Performance_SubByRun,1),...
        mean(Data.PDT_Peripheral.Objective_Performance_SubByRun,1),...
        mark*8.5,gradData,'filled','s');
    set(dualsolo,'MarkerEdgeColor','k');
    
    % 'Target' point representing partial dual-task means along with SEMs
    h = errorbar(mean(Data.PDT_Central.Objective_Performance_MEAN), mean(Data.PDT_Peripheral.Objective_Performance_MEAN),...
        mean(Data.PDT_Peripheral.Objective_Performance_SEM),'ks');
    set(h,'MarkerSize',mark+2,'LineWidth',2);
    set(h,'MarkerFaceColor','w');
    ...
    h = herrorbar(mean(Data.PDT_Central.Objective_Performance_MEAN), mean(Data.PDT_Peripheral.Objective_Performance_MEAN),...
        mean(Data.PDT_Central.Objective_Performance_SEM),'ks',clear_tees);
    set(h,'MarkerSize',mark+2,'LineWidth',2)
    set(h,'MarkerFaceColor','w');
...
title([Data.Experiment ' Objective Performance (n=8)'],'FontSize',16);
...
if SAVEIT == 1
    ...
    saveas(gcf,[FigurePath 'All_Objective_Performance' patch_string clear],'fig');
    % saveas(gcf,[FigurePath 'All_Objective_Performance'],'eps');
    
    original_dir = pwd;
    cd(FigurePath)
    
    print([Data.Experiment '_All_Objective_Performance' patch_string clear],'-depsc','-tiff')
